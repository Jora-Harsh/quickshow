...
/* ----------------------------------------------------
   PDFMAKE FONT FIX (Works on Railway & Linux safely)
---------------------------------------------------- */
pdfFonts.pdfMake = pdfFonts.pdfMake || {};
pdfFonts.pdfMake.vfs = pdfFonts.pdfMake.vfs || pdfFonts.vfs;

// Create printer using built‑in Roboto fonts
...
// Attach VFS to printer to load fonts correctly
...
/* --------------------------------------
   USERS EXCEL + PDF (YOUR ORIGINAL LOGIC)
-------------------------------------- */
// (YOUR USERS EXCEL/PDF ROUTES REMAIN UNCHANGED — they work fine)


// --- USERS PDF ROUTE PATCHED BELOW USING NEW PRINTER ---
router.get("/users/pdf", async (req, res) => {
  try {
    const filter = buildDateFilter(req);
    const users = await User.find(filter).sort({ createdAt: -1 });
    ...
    const docDef = {
      ...
      content:[
        {
          table:{ headerRows:1, widths: tableWidthsAuto(["sr","name","email","created"]), body },
        }
      ],
      styles:{
