...
export default function Releases() {
  ...
  const { nowPlaying, comingSoon } = useMemo(() => {
    ...
    let now = withDate.filter(
      (m) => m._rd && m._rd <= today && daysBetween(today, m._rd) <= PAST_WINDOW_DAYS
    );

    let soon = withDate.filter(
      (m) => m._rd && m._rd > today && daysBetween(m._rd, today) <= FUTURE_WINDOW_DAYS
    );

    if (!now.length && !soon.length) now = [...withDate];

    now.sort((a, b) => (b._rd?.getTime() ?? 0) - (a._rd?.getTime() ?? 0));
    soon.sort((a, b) => (a._rd?.getTime() ?? Infinity) - (b._rd?.getTime() ?? Infinity));
    ...
    <div className="max-w-6xl mx-auto px-6 md:px-8 pt-[120px] md:pt-[160px]">
      <div className="flex items-center justify-between gap-4 mb-6">
        ...
        <div className="inline-flex rounded-full overflow-hidden border border-white/10">
          <button
            onClick={() => setTab("now")}
            className={`px-4 py-2 text-sm ${tab === "now" ? "bg-primary text-white" : "bg-transparent"}`}
          >
            New & In Theaters
          </button>
          <button
            onClick={() => setTab("soon")}
            className={`px-4 py-2 text-sm ${tab === "soon" ? "bg-primary text-white" : "bg-transparent"}`}
          >
            Coming Soon
          </button>
        </div>
