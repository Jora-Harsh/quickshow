...
// useEffect(() => {
//   if (user) {
//     getMyBookings(); // fetch on mount

//     // Optional: poll every 10s to reflect payment immediately
...
const MyBookings = () => {
  ...
  const getMyBookings = async () => {
    try {
      const { data } = await axios.get(`/api/bookings/my-bookings`, { withCredentials: true });
      if (data.success) setBookings(data.bookings);
    } catch (err) {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Poll every 10 seconds to update payment status
      const interval = setInterval(getMyBookings, 10000);
      return () => clearInterval(interval);
    }
  }, [user]);

  // ✅ Refresh bookings immediately after Stripe redirect
  useEffect(() => {
          ...
          >
            ...
            <div className="flex flex-col md:flex-row flex-1">
              ...
              <div className="flex flex-col p-3 flex-1">
                ...
                <p className="text-gray-400 text-sm mt-1">{timeFormate(item.show.movie.runtime)}</p>
